//
//  OldAuthenticationService.swift
//  DoYourTasks
//
...
final class OldAuthenticationService: ObservableObject {
    ...
    func link(with email: String, password: String, completion: @escaping (Result<Void, OldAuthenticationServiceError>) -> Void) {
        ...
        user.link(with: authCredential) { authResult, error in
            if let error = error as NSError? {
                ...
                case .providerAlreadyLinked:
                    completion(.failure(.providerAlreadyLinked))
...
//            self.storeUser(user)
    ...
    func signIn(with email: String, password: String, completion: @escaping (Result<User, OldAuthenticationServiceError>) -> Void) {
    ...
    func sendPasswordReset(to email: String, completion: @escaping (Result<Void, OldAuthenticationServiceError>) -> Void) {
    ...
    func updatePassword(to password: String, completion: @escaping (Result<Void, OldAuthenticationServiceError>) -> Void) {
        guard let user = Auth.auth().currentUser else {
            completion(.failure(.unknownError))
            return
        }
    ...
    func updateEmail(to email: String, completion: @escaping (Result<Void, OldAuthenticationServiceError>) -> Void) {
        guard let user = Auth.auth().currentUser else {
            completion(.failure(.unknownError))
            return
        }
    ...
    func deleteAccount(completion: @escaping (Result<Void, OldAuthenticationServiceError>) -> Void) {
        guard let user = Auth.auth().currentUser else {
            completion(.failure(.unknownError))
            return
        }
...
enum OldAuthenticationServiceError: Error {
